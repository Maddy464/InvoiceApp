sap.ui.define([
...
], function( Controller,JSONModel, formatter, mobileLibrary,History) {
	"use strict";
	...
	return Controller.extend("invoiceui.controller.controller.Detail", {
		...
		_onObjectMatched : function (oEvent) {
			...
			let sPath = oArguments.ID;

			var oTable = this.byId("items12");
			sPath = '/InvoiceHeaders('+sPath+')'

			 var oItemTemplate = this.getView().byId('_IDGenColumnListItem112').clone();
                oTable.bindItems(sPath+"/to_items", oItemTemplate);

				this.byId("_IDGenHorizontalLayout").bindContext({ "path": sPath });



				// var oDetailPage = this.byId("page_detail");
                // oDetailPage.bindElement({ path: sPath })

				//  this.getView().setModel(this.getOwnerComponent().getModel());
                //   this.getView().byId("page_detail").bindContext({ "path": sPath });
      ...
      if (sPreviousHash !== undefined) {
        ...
        this.getOwnerComponent().getRouter().navTo("RouteWorkList", {}, undefined, true);
      }
    }
